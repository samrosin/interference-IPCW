...
arg <- Sys.getenv("SLURM_ARRAY_TASK_ID") ## get value of array ID 
set.seed(arg) ### set the arg as the seed
...
sim_dataset <- function(n_i = n_i,
  ...
  r1_is <- rnorm(n=n/n_i, mean=0, sd=.1) 
  ...
  df.new <- data.frame(id=seq(1,n_i*m),group=df$group,L1_ij= df$L1_ij,L2_ij= df$L2_ij)
  ...
  ####drop a few columns. these can be added back for debugging 
...
simmed.dataset <- sim_dataset(n_i=n_i, m=m)
base.dataset <- simmed.dataset[[1]]; potential.outcomes <- simmed.dataset[[2]]

base_wd <- paste(user_home_directory,"base_datasets/base_datasets_ni",
                 n_i,"_m",m,sep="")
setwd(base_wd)
base_filename <- paste("base_",arg,".csv",sep="")
write.csv(x = base.dataset, file = base_filename, row.names = FALSE)
...
truth <- calc_estimands(potential.outcomes, alphas, time.until = 100)
truth_wd <- paste(user_home_directory,"truthtables/truths_ni",n_i,"_m",m,sep="")
setwd(truth_wd)
truth_filename <- paste("truth_",arg,".csv",sep="")
write.csv(x = truth, file = truth_filename, row.names = FALSE)
