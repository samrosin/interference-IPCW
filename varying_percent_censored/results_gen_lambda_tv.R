...
# Tchetgen Tchetgen and Vanderweele (TV) point estimates and ASEs. 
# This is done using the array method in SLURM
...
results_dirpath <- paste(dir_path, "results_datasets_tv/",sep="")
print(results_dirpath)
dir.create(results_dirpath)
output_filename <- paste("results_tv_",subdir_name,"_",arg,".csv",sep="")
  ...
  function(theta, alphas, time.until){
    ...
    fhats.gen <- function(alpha){
      ...
      propensity.fn <- Vectorize(function(b){
        ...
        dn <- dnorm(x=b,mean=0,sd=theta[4])
        if(dn==0){return(0)} 
        return(exp(sum(
      ...
      #wrap up
      fhat0 <- (1/nrow(data))*sum(data$numer0/p.hat)
      fhat1 <- (1/nrow(data))*sum(data$numer1/p.hat)
    ...
    theta0.ixs <- 5:(5+alphas.len-1)
    theta1.ixs <- (5+alphas.len):(5+2*alphas.len-1)
    
    return(c(prop.scores(theta[1:4]),
             fhats[,1] - theta[theta0.ixs], fhats[,2] - theta[theta1.ixs]))
  }
}

### Estimate the vcov using geex, using the propensity model
...
ase_est_call <- function(base, theta, alphas, time.until, p.mixed){
  ...
  grab.ases <- function(a){
    if(a==0){
      ixs <- 5:(4+length(alphas)) ### values 5,4 have to do with length of theta vector, and where the causal parameters are in the vcov. 4 is length of theta vector. may need to adjust by length of theta vector. 
    } else {
      if(a==1){ ixs <- (5+length(alphas)):(4+length(alphas)*2)} ### values 5,4 have to do with length of theta vector, and where the causal parameters are in the vcov. careful. 
    } 
...
ase_est <- function(base, alphas, p.mixed,
                    mu.mat, time.until){
  ...
  theta <- as.vector(c(thetas.prop,muhat.0s, muhat.1s))
...
fhat_i_aalpha <- function(grpdt, alpha, time.until = 100,
                          theta.s, p.mixed, fixefs
){
  #numerators for the estimators under a=1 and a=0
  grpdt$alpha.denom <- alpha^grpdt$A_ij*(1-alpha)^(1-grpdt$A_ij)
  grpdt$numer1 <- (1/grpdt$alpha.denom)*grpdt$A_ij*(grpdt$X_ij<time.until)
  grpdt$numer0 <- (1/grpdt$alpha.denom)*(1-grpdt$A_ij)*(grpdt$X_ij<time.until)
  ...
  #wrap up
  fhat.0 <- (1/nrow(grpdt))*sum(grpdt$numer0/p.hat)
  fhat.1 <- (1/nrow(grpdt))*sum(grpdt$numer1/p.hat)
  return(c(fhat.0,fhat.1))
}

##### evaluate ipcw estimator for a specific alpha, simulated dataset called "base"
ipcw.eval.alpha <- function(base, alpha, fixefs,theta.s,
                            p.mixed, time.until = 100){
...
ipcw.eval.allalphas <- function(base, alphas, time.until){
  ...
  all.ipcws <- lapply(X=alphas,FUN=ipcw.eval.alpha, base=base,
                      fixefs=fixefs, theta.s=theta.s, 
                      p.mixed=p.mixed, time.until=time.until)
  all.ipcws <- matrix(unlist(all.ipcws),ncol=2,byrow=TRUE)
  ases <- ase_est(base,alphas,p.mixed,
                  mu.mat = all.ipcws,time.until = 100)
  cens.pct <- mean(1-base$Delta_ij) #percentage of individuals who were censored
  all.results <- cbind(all.ipcws, ases, cens.pct)
  print(all.results)
  
  rownames(all.results) <- alphas
  colnames(all.results) <- c("mu-hat0","mu-hat1","ASE0","ASE1","cens_pct")
  return(all.results)
}
