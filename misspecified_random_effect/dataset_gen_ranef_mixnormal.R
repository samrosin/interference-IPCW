...
sim_dataset <- function(n_i = n_i,
  ...
  #####         same random effect.
  #### Here they are generated from a 50:50 mixture of Normal distributions,
  #### i.e. with probability .5 b_i ~ N(-1, sd=.3) and 
  #### with probability .5 b_i ~ N(1, sd=.3) ,
  #### such that the variance is approximately 1.09
  ####
  mix <- rbinom(n = n/n_i, 1, .5)
  b_is <- (1-mix)*rnorm(n/n_i,-1,0.3)+mix*rnorm(n/n_i,1,0.3) 
