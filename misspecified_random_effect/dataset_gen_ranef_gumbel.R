...
library(evd, lib.loc = libs) #extreme value dist package for Gumbel distribution function
...
sim_dataset <- function(n_i = n_i,
  ...
  #####         same random effect.
  #### Here they are generated from a mean-zero Gumbel distribution 
  #### with variance 1.0859. 
  #### this corresponds to a location parameter of 
  #### mu= digamma(1)*.8124949 = -0.4689848
  #### and a scale parameter of 
  #### beta = sqrt(6.5154)/pi = .8124949. 
  ####
  #### This is because mean = mu + gamma*beta, 
  #### and variance = pi^2*beta^2/6, 
  #### where gamma is the Euler-Mascheroni constant = -digamma(1)
  ####
  b_is <- rgumbel(n=n/n_i, loc = -0.4689848, scale = 0.8124949)
